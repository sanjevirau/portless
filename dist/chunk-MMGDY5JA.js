...
`;
  ...
  const handleH2WebSocket = (stream, headers) => {
    stream.on("error", () => stream.destroy());
    const authority = headers[":authority"] || "";
    const host = authority.split(":")[0];
    const path3 = headers[":path"] || "/";
    try {
    ...
    const hops = parseInt(headers[PORTLESS_HOPS_HEADER], 10) || 0;
    if (hops >= MAX_PROXY_HOPS) {
      onError(
    ...
    const wsKey = headers["sec-websocket-key"] || crypto.randomBytes(16).toString("base64");
    const fakeReq = {
    ...
    if (headers["sec-websocket-protocol"]) {
      backendHeaders["sec-websocket-protocol"] = headers["sec-websocket-protocol"];
    }
    if (headers["sec-websocket-extensions"]) {
      backendHeaders["sec-websocket-extensions"] = headers["sec-websocket-extensions"];
    }
    if (headers["origin"]) {
      backendHeaders["origin"] = headers["origin"];
    }
    if (headers["user-agent"]) {
      backendHeaders["user-agent"] = headers["user-agent"];
    }
    if (headers["cookie"]) {
      backendHeaders["cookie"] = headers["cookie"];
    }
    for (const [key, value] of Object.entries(forwardedHeaders)) {
    ...
    const onBackendData = (chunk) => {
      ...
      const acceptHeader = lines.slice(1).map((l) => l.split(":")).find(([k]) => k && k.trim().toLowerCase() === "sec-websocket-accept");
